				...
				for (int top_h = 0; top_h <= height - roof_h; top_h += 0.5 * step_size){
					...
					for (int roof_w_v1 = roof_min_size; roof_w_v1 <= width; roof_w_v1 += step_size){
						for (int roof_h_v1 = roof_min_size; roof_h_v1 <= height; roof_h_v1 += step_size){
							for (int top_w_v1 = 0; top_w_v1 <= width - roof_w_v1; top_w_v1 += 0.5 * step_size){
								for (int top_h_v1 = 0; top_h_v1 <= height - roof_h_v1; top_h_v1 += 0.5 * step_size){
		...
		for (int roof_h = roof_min_size; roof_h <= height; roof_h += step_size){
			for (int top_w = 0; top_w <= width - roof_w; top_w += step_size){
				for (int top_h = 0; top_h <= height - roof_h; top_h += step_size){
					...
					/*if (!utils::rectInsideRect(width, height, center_w, center_h, roof_w, roof_h))
						continue;*/
					if (roof_w > 0.8 * width && roof_h > 0.8 * height)
						continue;
					if (roof_w < roof_h )
						continue;
					roof_paras[0].clear();
						...
						for (int roof_h_v1 = roof_min_size; roof_h_v1 <= height; roof_h_v1 += step_size){
							for (int top_w_v1 = 0; top_w_v1 <= width - roof_w_v1; top_w_v1 += step_size){
								for (int top_h_v1 = 0; top_h_v1 <= height - roof_h_v1; top_h_v1 += step_size){
									...
									/*if (!utils::rectInsideRect(width, height, center_w_v1, center_h_v1, roof_w_v1, roof_h_v1))
										continue;*/
									if (roof_w_v1 > 0.5 * width && roof_h_v1 > 0.5 * height)
										continue;
									roof_paras[1].clear();
									roof_paras[1].push_back(center_w_v1);
									roof_paras[1].push_back(center_h_v1);
									...
									for (int roof_w_v2 = roof_min_size; roof_w_v2 <= width; roof_w_v2 += step_size){
										for (int roof_h_v2 = roof_min_size; roof_h_v2 <= height; roof_h_v2 += step_size){
											for (int top_w_v2 = 0; top_w_v2 <= width - roof_w_v2; top_w_v2 += step_size){
												for (int top_h_v2 = 0; top_h_v2 <= height - roof_h_v2; top_h_v2 += step_size){
													...
													/*if (!utils::rectInsideRect(width, height, center_w_v2, center_h_v2, roof_w_v2, roof_h_v2))
														continue;*/
													if (roof_w_v2 > 0.5 * width && roof_h_v2 > 0.5 * height)
														continue;
													roof_paras[2].clear();
												...
												for (int top_h_v2 = 0; top_h_v2 <= height - roof_h_v2; top_h_v2 += step_size){
													...
													for (int roof_w_v3 = roof_min_size; roof_w_v3 <= width; roof_w_v3 += step_size){
														for (int roof_h_v3 = roof_min_size; roof_h_v3 <= height; roof_h_v3 += step_size){
															for (int top_w_v3 = 0; top_w_v3 <= width - roof_w_v3; top_w_v3 += step_size){
																for (int top_h_v3 = 0; top_h_v3 <= height - roof_h_v3; top_h_v3 += step_size){
		...
		for (int roof_h = roof_min_size; roof_h <= height; roof_h += step_size){
			for (int top_w = 0; top_w <= width - roof_w; top_w += 0.5 * step_size){
				for (int top_h = 0; top_h <= height - roof_h; top_h += 0.5 * step_size){
					...
					/*if (!utils::rectInsideRect(width, height, center_w, center_h, roof_w, roof_h))
						continue;*/
					if (roof_w > 0.4 * width && roof_h > 0.4 * height)
						continue;
					if (roof_w < roof_h)
						continue;
					roof_paras[0].clear();
						...
						for (int roof_h_v1 = roof_min_size; roof_h_v1 <= height; roof_h_v1 += step_size){
							for (int top_w_v1 = 0; top_w_v1 <= width - roof_w_v1; top_w_v1 += 0.5 * step_size){
								for (int top_h_v1 = 0; top_h_v1 <= height - roof_h_v1; top_h_v1 += 0.5 * step_size){
									...
									if (!utils::rectInsideRect(width, height, center_w_v1, center_h_v1, roof_w_v1, roof_h_v1))
										continue;
									if (roof_w_v1 > 0.4 * width && roof_h_v1 > 0.4 * height)
										continue;
									roof_paras[1].clear();
									roof_paras[1].push_back(center_w_v1);
									roof_paras[1].push_back(center_h_v1);
									...
									for (int roof_w_v2 = roof_min_size; roof_w_v2 <= width; roof_w_v2 += step_size){
										for (int roof_h_v2 = roof_min_size; roof_h_v2 <= height; roof_h_v2 += step_size){
											for (int top_w_v2 = 0; top_w_v2 <= width - roof_w_v2; top_w_v2 += 0.5 * step_size){
												for (int top_h_v2 = 0; top_h_v2 <= height - roof_h_v2; top_h_v2 += 0.5 * step_size){
													int center_w_v2 = top_w_v2 + 0.5 * roof_w_v2;
													int center_h_v2 = top_h_v2 + 0.5 * roof_h_v2;
													if (!utils::rectInsideRect(width, height, center_w_v2, center_h_v2, roof_w_v2, roof_h_v2))
														continue;
													if (roof_w_v2 > 0.4 * width && roof_h_v2 > 0.4 * height)
														continue;
													roof_paras[2].clear();
													roof_paras[2].push_back(center_w_v2);
													roof_paras[2].push_back(center_h_v2);
														...
														for (int roof_h_v3 = roof_min_size; roof_h_v3 <= height; roof_h_v3 += step_size){
															for (int top_w_v3 = 0; top_w_v3 <= width - roof_w_v3; top_w_v3 += step_size){
																for (int top_h_v3 = 0; top_h_v3 <= height - roof_h_v3; top_h_v3 += step_size){
																	...
																	if (!utils::rectInsideRect(width, height, center_w_v3, center_h_v3, roof_w_v3, roof_h_v3))
																		continue;
																	if (roof_w_v3 > 0.4 * width && roof_h_v3 > 0.4 * height)
																		continue;
																	roof_paras[3].clear();
																	roof_paras[3].push_back(center_w_v3);
																	roof_paras[3].push_back(center_h_v3);
				...
				for (int top_h = 0; top_h <= height - roof_h; top_h += 0.5 * step_size){
					...
					if (roofType == RoofTypes::FLAT){
						...
						for (int iter = 0; iter < 500; iter++){
							int center_w_tmp = center_w + utils::genRand(-dis_ratio, dis_ratio);
							int center_h_tmp = center_h + utils::genRand(-dis_ratio, dis_ratio);
							int roof_w_tmp = roof_w + utils::genRand(-dis_ratio, dis_ratio);
							int roof_h_tmp = roof_h + utils::genRand(-dis_ratio, dis_ratio);
							cv::Mat roof_img(height, width, CV_8UC3, bg_color);
						...
						for (int iter = 0; iter < 500; iter++){
							...
							int roof_w_tmp = roof_w + utils::genRand(-dis_ratio, dis_ratio);
							int roof_h_tmp = roof_h + utils::genRand(-dis_ratio, dis_ratio);
							DrawRotatedRect::generateRect(roof_img, padding, center_w_tmp, center_h_tmp, roof_w_tmp, roof_h_tmp, 0, RoofTypes::HIP, true, 0.03, roof_w > roof_h ? roof_w_tmp : roof_h_tmp, bg_color, fg_color);
							if (!roof_img.empty()){
								char buffer[50];
								sprintf(buffer, "roof_image_%06d.png", index);
								std::string img_filename = output_path + "/" + std::string(buffer);
						...
						int dis_ratio = 5;
						for (float ridge_ratio = 0.4; ridge_ratio < 0.8; ridge_ratio += 0.02){
							for (int iter = 0; iter < 25; iter++){
								...
								int roof_w_tmp = roof_w + utils::genRand(-dis_ratio, dis_ratio);
								int roof_h_tmp = roof_h + utils::genRand(-dis_ratio, dis_ratio);
								DrawRotatedRect::generateRect(roof_img, padding, center_w_tmp, center_h_tmp, roof_w_tmp, roof_h_tmp, 0, RoofTypes::HIP, false, 0, roof_w_tmp > roof_h_tmp ? roof_w_tmp * ridge_ratio : roof_h_tmp * ridge_ratio, bg_color, fg_color);
								if (!roof_img.empty()){
									char buffer[50];
									sprintf(buffer, "roof_image_%06d.png", index);
									std::string img_filename = output_path + "/" + std::string(buffer);
						...
						for (float ridge_ratio = 0.4; ridge_ratio < 0.8; ridge_ratio += 0.02){
							for (int iter = 0; iter < 25; iter++){
								...
								int roof_w_tmp = roof_w + utils::genRand(-dis_ratio, dis_ratio);
								int roof_h_tmp = roof_h + utils::genRand(-dis_ratio, dis_ratio);
								DrawRotatedRect::generateRect(roof_img, padding, center_w_tmp, center_h_tmp, roof_w_tmp, roof_h_tmp, 0, RoofTypes::GABLE_HIP, false, 0, roof_w_tmp > roof_h_tmp ? roof_w_tmp * ridge_ratio : roof_h_tmp * ridge_ratio, bg_color, fg_color);
								if (!roof_img.empty()){
									char buffer[50];
									sprintf(buffer, "roof_image_%06d.png", index);
									std::string img_filename = output_path + "/" + std::string(buffer);
