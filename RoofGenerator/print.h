...
template<class K>
std::vector<std::vector<double>> edges_straight_skeleton(CGAL::Straight_skeleton_2<K> const& ss)
{
	typedef CGAL::Straight_skeleton_2<K> Ss;
	...
	for (Halfedge_const_iterator i = ss.halfedges_begin(); i != ss.halfedges_end(); ++i)
	{
		if (i->is_bisector()){
			...
			tmp.push_back(i->opposite()->vertex()->point().x());
			tmp.push_back(i->opposite()->vertex()->point().y());
			tmp.push_back(i->vertex()->point().x());
			tmp.push_back(i->vertex()->point().y());
			edges.push_back(tmp);
		}
	}
	return edges;
