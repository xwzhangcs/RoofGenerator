...
void test_two_nodes_hip(int width, int height, int step_size, int type, int padding);

int main(int argc, char** argv)
{
	int padding = 5;
	//test_one_nodes(64, 64, 8, padding);
	test_two_nodes_hip(64, 64, 8, 1, padding);
	system("pause");
		...
		for (int roof_h = roof_min_size; roof_h <= height; roof_h += step_size){
			for (int top_w = 0; top_w < width; top_w += 0.5 * step_size){
				for (int top_h = 0; top_h < height; top_h += 0.5 * step_size){
					...
					roof_paras[0].push_back(roof_w > roof_h ? roof_w * ridge_ratio : roof_h * ridge_ratio);
					// check 
					// second rectangle
							...
							for (int top_w_v1 = 0; top_w_v1 < width; top_w_v1 += 0.5 * step_size){
								for (int top_h_v1 = 0; top_h_v1 < height; top_h_v1 += 0.5 * step_size){
									...
									if (!roof_img.empty()){
										char buffer[50];
										sprintf(buffer, "roof_image_%08d.png", index);
										std::string img_filename = "../data/two_edges_gable/" + std::string(buffer);
										std::cout << img_filename << std::endl;
										cv::imwrite(img_filename, roof_img);
										index++;
									}
		...
		for (int roof_h = roof_min_size; roof_h <= height; roof_h += step_size){
			for (int top_w = 0; top_w < width; top_w += 0.5 * step_size){
				for (int top_h = 0; top_h < height; top_h += 0.5 * step_size){
					...
					if (roof_w > roof_h){
						// second rectangle
						int roof_w_v1 = (int)(0.4 * roof_w) % 2 == 0 ? (int)(0.4 * roof_w) : (int)(0.4 * roof_w) + 1;
						for (int roof_h_v1 = roof_min_size; roof_h_v1 <= height; roof_h_v1 += step_size){
							for (int top_w_v1 = 0; top_w_v1 < width; top_w_v1 += 0.5 * step_size){
								for (int top_h_v1 = 0; top_h_v1 < height; top_h_v1 += 0.5 * step_size){
									int center_w_v1 = top_w_v1 + 0.5 * roof_w_v1;
									...
									roof_paras[1].push_back(roof_h_v1);
									roof_paras[1].push_back(0);
									roof_paras[1].push_back(roof_w_v1 > roof_h_v1 ? roof_w_v1 * ridge_ratio : roof_h_v1 * ridge_ratio);
									cv::Mat roof_img(height, width, CV_8UC3, bg_color);
									DrawRotatedRect::generateRect(roof_img, padding, roof_paras, RoofRelationship::L, RoofTypes::HIP, bg_color, fg_color);
									if (!roof_img.empty()){
										char buffer[50];
										sprintf(buffer, "roof_image_%08d.png", index);
										std::string img_filename = "../data/two_edges_hip/" + std::string(buffer);
										std::cout << img_filename << std::endl;
										cv::imwrite(img_filename, roof_img);
										index++;
									}
