...
cv::Mat RoofA::generateRoof(int width, int height, const Config& roof_paras, const cv::Scalar& bg_color, const cv::Scalar& fg_color){
	...
	if (roof_paras.selected_roof_type == RoofTypes::FLAT){
		...
		DrawRotatedRect::generateRect(result, center_w, center_h, imageRoofWidth, imageRoofHeight, roof_paras.rotate, roof_paras.selected_roof_type, roof_paras.bRidgeDis, 0.10, ridge_length, bg_color, fg_color);
	}
	else{
		// do nothing
	}
