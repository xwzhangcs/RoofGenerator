...
cv::Mat RoofD::generateRoof(int width, int height, const std::vector<Config>& roof_paras, int padding, const cv::Scalar& bg_color, const cv::Scalar& fg_color, int type, bool bDebug){
	...
	else if (type == 1) // 0 and 1 are connected, 2, 3 are separated.
	{
		for (int i = 0; i < roof_paras.size(); i++){
			for (int j = i + 1; j < roof_paras.size(); j++){
				if (i == 0 && j == 1){
